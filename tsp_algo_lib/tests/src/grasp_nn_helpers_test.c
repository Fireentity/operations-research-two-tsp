...
static void test_grasp_construction_basic(void) {
    ...
    int res = grasp_nearest_neighbor_tour(0, tour, n, costs, &cost, 3, 0.5, &rng);
...
static void test_grasp_invalid_start_node(void) {
    ...
    int res = grasp_nearest_neighbor_tour(10, tour, n, costs, &cost, 3, 0.5, &rng);
    assert(res == -1);
}

void run_grasp_nn_helpers_tests(void) {
    printf("[GRASP Helper] Running tests...\n");
    random_init(&rng, global_random_int(0,INT_MAX));
