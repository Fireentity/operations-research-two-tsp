...
Variable Neighborhood Search relies on the systematic change of neighborhood structures to escape local optima.
The central observation is that a solution locally optimal with respect to a specific neighborhood structure (e.g., the 2-OPT swap) is not necessarily optimal with respect to another (e.g., a larger $k$-OPT perturbation).
By alternating between a \("\)shaking\("\) phase to move the solution to a new basin of attraction and a "descent" phase to find the local optimum within that new basin, VNS effectively balances diversification and intensification.

\paragraph{The Shaking Mechanism (Kick)}
The pivotal component of this strategy is the \("\)kick\("\) operator, designed to forcefully disrupt the stability of a local optimum.
As illustrated in Figure~\ref{fig:vns_kick_explanation}, a local optimum (such as the clean perimeter on the left) represents a valley in the fitness landscape from which simple local moves cannot escape without temporarily increasing the cost.
The kick operator overcomes this by applying a stochastic $k$-OPT perturbation.
As detailed in Figure~\ref{fig:vns_kick_steps}, the algorithm selects $k$ arbitrary edges to cut (Step 1), effectively decomposing the tour into independent segments (Step 2).
These segments are then reconnected in a scrambled order (Step 3), creating a solution that is structurally distinct and often geometrically \("\)chaotic\("\) (e.g., introducing multiple crossings).
Although this new candidate initially has a worse objective function value, it provides a fresh starting point for the local search to descend into a potentially deeper and unexplored valley.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figures/6-3opt-graph}
    \caption{Visual effect of a 3-OPT Kick. Left: The current local optimum features a clean, crossing-free geometry. Right: The kick perturbs the solution by randomly reconnecting segments, creating a "crossed" structure that degrades the cost but allows exploration of new subspaces.}
    \label{fig:vns_kick_explanation}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figures/6-3opt-procedure}
    \caption{Logical mechanics of the Kick operator. (1) $k$ edges (green) are selected for removal. (2) The tour is decomposed into independent segments. (3) The segments are reconnected using new edges (red dashed lines) to form a valid, perturbed tour.}
    \label{fig:vns_kick_steps}
\end{figure}

\paragraph{Implementation Strategy and Hyperparameters}
Our implementation, detailed in Algorithm~\ref{alg:vns}, utilizes a dynamic neighborhood size $k$ that scales from a lower bound $k_{\min}$ to an upper bound $k_{\max}$.
