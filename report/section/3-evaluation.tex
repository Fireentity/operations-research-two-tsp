...
To avoid manual editing and to keep the benchmarking pipeline reproducible, we implemented an automated data recording system. This system programmatically aggregates results from multiple solver executions into a unified dataset, avoiding error-prone manual entry. This design makes it straightforward to run parameter sweeps, record results consistently, and immediately generate the corresponding performance profiles. For example, when tuning the Tabu Search, distinct columns are automatically generated for each configuration (e.g., \texttt{TS\_tenure10}, \texttt{TS\_tenure20}), allowing for a direct side-by-side comparison of solution quality across all benchmark instances.
