...
FlagParser* cmd_options_build_cli_parser(CmdOptions* opt) {
    ...
    flag_parser_add_uint(p, "--square-side", NULL, "Generation area side", &opt->tsp.generation_area.square_side, FLAG_OPTIONAL);
    ...
    flag_parser_add_uint(p, "--ts-stagnation", NULL, "TS Max Stagnation", &opt->tabu_params.max_stagnation, FLAG_OPTIONAL);
    ...
    flag_parser_add_string_owned(p, "--grasp-plot", NULL, "GRASP plot file", &opt->grasp_params.plot_file, FLAG_OPTIONAL);
    flag_parser_add_string_owned(p, "--grasp-cost", NULL, "GRASP cost file", &opt->grasp_params.cost_file, FLAG_OPTIONAL);
