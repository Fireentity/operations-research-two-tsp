...
const ParsingResult *flag_parser_parse(FlagParser *parser,
    ...
    // TODO make better, like the -- or - is too specific i think
    for (int i = 0; i < argc; i++) {
        ...
        int def_index = find_flag_by_name(parser, arg);
        if (def_index == -1) {
            if (skip_unknowns) continue;
            return UNKNOWN_ARG;
        }

        FlagDefinition *def = parser->definitions[def_index];
...
void flag_parser_print_help(const FlagParser *parser) {
    // TODO Make this more sophisticated (e.g., wrap lines)
    printf("Usage: [program] [options]\n\nOptions:\n");

    for (size_t i = 0; i < parser->definition_count; i++) {
        const FlagDefinition *def = parser->definitions[i];
        char short_name_str[5] = "    "; // 4 spaces for alignment

        if (def->short_name) {
            snprintf(short_name_str, 5, "%s, ", def->short_name);
        }

        // Print: "  -s, --short    Description"
        printf("  %s%s\t%s\n",
               short_name_str,
               def->name,
