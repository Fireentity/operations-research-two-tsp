...
struct hashmap* init_hashmap(const Flag** flags, const int number_of_flags)
{
    struct hashmap* map = hashmap_new(sizeof(FlagEntry),
                                      0,
                                      rand(),
                                      rand(),
                                      flag_entry_hash,
...
ParsingResult parse_flags(CmdOptions* cmd_options,
                          ...
                          const char** argv)
{
    const int mandatory_flags = count_number_of_flags_if(tsp_flags, number_of_flags, is_mandatory);
    // Count required flags
    ...
    for (unsigned int current_param = 1; current_param < argc - 1; current_param++)
    {
        ...
        const ParsingResult result = found_entry->flag->parse(found_entry->flag, cmd_options, argv, &current_param);
